/*
 * STM_F103C6_EXTI_Driver.h
 *
 *  Created on: Jun 28, 2022
 *      Author: Mohamed Adel
 */

#ifndef INC_STM_F103C6_EXTI_DRIVER_H_
#define INC_STM_F103C6_EXTI_DRIVER_H_
...
typedef struct
{
	uint16_t EXT_Input_line_Number ;
...
typedef struct
{
	//specify the external GPIO mapping
	//This parameter is set based on @ref EXTI_define
	EXTI_GPIO_Mapping_t EXTI_pin;

	//Specify the trigger case
	//This parameter is set based on @ref trigger_case_define
	uint8_t Trigger_Case ;

	//Enable or Disable the EXTI IRQ (This will enable the IRQ Mask in EXTI and also enable the NVIC interrupt)
	//This parameter is set based on @ref IRQ_EN_define
	uint8_t IRQ_EN ;

	//Set the C Function() which will be called once the IRQ happens
